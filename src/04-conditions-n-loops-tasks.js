...
function isTriangle(a, b, c) {
  if (2 * Math.max(a, b, c) - a - b - c >= 0) return false;
  return true;
}
...
function doRectanglesOverlap(rect1, rect2) {
  if (rect1.top >= rect2.top && rect1.left >= rect2.left && rect1.top <= rect2.top + rect2.height
    && rect1.left <= rect2.left + rect2.width) return true;
  if (rect2.top >= rect1.top && rect2.left >= rect1.left && rect2.top <= rect1.top + rect1.height
...
function isCreditCardNumber(ccn) {
  const line = ccn.toString(10);
  let sum = 0;
  let parrity;
  if (line.length % 2 !== 0) parrity = 1;
  else parrity = 0;
  for (let i = line.length - 1; i >= 0; i -= 1) {
    if ((i + parrity) % 2 === 0) {
      let numb = parseInt(line[i], 10) * 2;
      if (numb > 9) numb -= 9;
      sum += numb;
    } else sum += parseInt(line[i], 10);
  }
...
function getDigitalRoot(num) {
  ...
  while (sum.length !== 1) {
    sum = sum.reduce((acc, currentValue) => {
...
function getCommonDirectoryPath(pathes) {
  const minLength = pathes.reduce((acc, currentValue) => {
    if (acc > currentValue.length) acc = currentValue.length;
    return acc;
  }, pathes[0].length);
  let res = '';
